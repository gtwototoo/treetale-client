<script lang="ts">
	...
	onMount(() => {
		if (stringSearchParams) value = stringSearchParams;
		if (genresSearchParams) searchedGenres = genresSearchParams;
		if (formatSearchParams) searchedFormat = formatSearchParams as StoryFormat;
...
<div class="flex grow justify-center overflow-hidden">
	<section class="flex w-full max-w-[120rem] flex-col gap-4 py-4">
		...
		{#if data.categories.length}
			<div class="flex w-full flex-col items-center gap-4 px-4">
				...
				<Input
					bind:value
					class={clm(button.size.xl, 'adaptive-font-upper w-full max-w-2xl pl-6')}
					oninput={handleInput}
					placeholder="Поиск"
				>
					{#snippet left()}
					...
					{#each GENRES_LIST as { icon: GenreIcon, id, title } (id)}
						<Button
							class={clm(
								button.type.primary,
								button.size.lg,
								'h-20 w-24 flex-col justify-center gap-1 py-0 max-md:h-16 max-sm:flex-1',
								searchedGenres.includes(id) && 'bg-main-500'
							)}
							onclick={() => switchGenre(id)}
						>
							<GenreIcon class="text-main size-8 max-md:size-6" />
